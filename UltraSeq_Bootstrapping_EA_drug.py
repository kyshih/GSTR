'''
 ...
 - Eml4 variants experiment has no sgInert. So for each bootstrapping cycle, I use calculate metrics relative to the meidan metric
 of all gRNAs (population wide)
'''
...
def Calculate_Relative_Normalized_Metrics(input_df1,input_df2,percentile_list,input_control_gRNA_list):
    ...
    # relative to population median of all gRNAs. no sgInert in this experiment
...
def Add_Corhort_Specific_Relative_Metrics(input_df,input_control_list):
    ...
    for temp_cname in input_df.drop(columns=['gRNA'],inplace = False).columns:
        ...
        input_df[temp_name] = input_df[temp_cname]/temp_sub[temp_cname].median() # inert
...
def Generate_Normalized_Metrics(input_df1,input_df2,trait_list):
    ...
    temp2 = input_df2.set_index('gRNA').reindex(temp1.index) # ensure that any gRNA in temp1 that is not found in input_df2 will result in NaN instead of KeyError
    # id the missing gRNAs
    missing_gRNAs = temp2[temp2.isna().any(axis=1)].index.tolist()
    print(f"Missing gRNAs: {missing_gRNAs}")
    # fill NaN with 0
    ...
    for temp_cname in trait_list:
        ...
        temp_output_df[temp_cname_new] = np.array(temp1[temp_cname].to_list())/np.array(temp2[temp_cname].to_list()) # PCE/KP
    return(temp_output_df)
...
def main():
    ...
    if number_of_bootstrap!=0:
        ...
        #temp_trait_list = ['LN_mean','Geo_mean','TTB_normalized','TTN_normalized','95_percentile'] + [str(x) + '_percentile' for x in temp_q]
