...
from shrinkage_analysis.find_S_matching_median_TTN import find_S
import numpy as np
import copy
import argparse
...
def find_significant_shrinkage_given_base_cutoff(raw_df, input_sample_list1, input_sample_list2, cell_number_cutoff_list, input_control_gRNA_list, number_of_replicate, input_total_gRNA_number):
    ...
    final_S_summary_df = Generate_Final_Shrinkage_Summary_Dataframe(final_S_df, ['Shrinkage'], 'basal_cutoff')
...
def main():
    ...
    final_interm_df.to_csv(f'{output_address}/S_intermediate_given_base_cutoff_{variant}', index=None)
    final_sum_df.to_csv(f'{output_address}/S_summary_given_base_cutoff_{variant}.csv', index=None)
